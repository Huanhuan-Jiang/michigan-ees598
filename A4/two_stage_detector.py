...
class RPNPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for n in range(num_layers):
            ...
            nn.init.normal_(stem_rpn[2*n].weight.data, mean=0.0, std=0.01)
            nn.init.constant_(stem_rpn[2*n].bias.data, 0)
...
def iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    boxes1 = boxes1.to(boxes2.device)
    #print(boxes1.device)
    #print(boxes2.device)
...
def rcnn_get_deltas_from_anchors(
    ...
    bn_mask = (gt_boxes[:, :4] == -1) | (gt_boxes[:, :4] == -1e8)
    deltas = deltas * (~bn_mask) - 1e8 * bn_mask
...
def rcnn_apply_deltas_to_anchors(
    ...
    output_boxes = output_boxes * (~bn_mask) - 1e8 * bn_mask
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
    return output_boxes
