...
def iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    ...
    def intersection(boxes1, boxes2):
        ...
        boxes1.to(boxes2.device)
        #print(boxes2.device)
        #print(boxes2.device)
        max_xy = torch.min(boxes1[:, 2:].unsqueeze(1).expand(M, N, 2).to(boxes2.device), boxes2[:, 2:].unsqueeze(0).expand(M, N, 2))
        min_xy = torch.max(boxes1[:, :2].unsqueeze(1).expand(M, N, 2).to(boxes2.device), boxes2[:, :2].unsqueeze(0).expand(M, N, 2))
    ...
    union = area_a.to(boxes2.device) + area_b.to(boxes2.device) - inter.to(boxes2.device)
...
class RPN(nn.Module):
    ...
    def forward(
        ...
        if self.training:
            ...
            #print(f"anchor_boxes device: {anchor_boxes.device}")
            #print(f"fg_idx device: {fg_idx.device}")
            #print(f"bg_idx device: {bg_idx.device}")
            #print(f"anchor_boxes shape: {anchor_boxes.shape}")
            
            anchor_boxes = anchor_boxes.to(fg_idx.device)
            #print(f"anchor_boxes device: {anchor_boxes.device}")
            #fg_idx.to(anchor_boxes.device)
            #bg_idx.to(anchor_boxes.device)
            #print(f"fg_idx shape: {fg_idx.shape}")
            #print(f"fg_idx device: {fg_idx.device}")
            #print(f"bg_idx shape: {bg_idx.shape}")
            #print(f"bg_idx device: {bg_idx.device}")
