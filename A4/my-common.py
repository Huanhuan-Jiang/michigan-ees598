...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def forward(self, images: torch.Tensor):
        ...
        # Replace "pass" statement with your code
        lateral_out_c5 = self.lateral_c5(backbone_feats["c5"])
        lateral_out_c4 = torch.add(F.interpolate(lateral_out_c5, scale_factor=2), self.lateral_c4(backbone_feats["c4"]))
        lateral_out_c3 = torch.add(F.interpolate(lateral_out_c4, scale_factor=2), self.lateral_c3(backbone_feats["c3"]))

        fpn_feats["p5"] = self.output_p3(lateral_out_c5)
        fpn_feats["p4"] = self.output_p3(lateral_out_c4)
        fpn_feats["p3"] = self.output_p3(lateral_out_c3)
